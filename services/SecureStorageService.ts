...
class SecureStorageService {
  ...
  async setItem(key: string, value: string): Promise<void> {
    try {
      ...
      if (__DEV__) {
        console.error(
          `❌ Erreur lors de la sauvegarde sécurisée de ${key}:`,
          error,
        );
      }
      throw error;
  ...
  async getItem(key: string): Promise<string | null> {
    try {
      ...
      console.error(
        `❌ Erreur lors de la récupération sécurisée de ${key}:`,
        error,
      );
      return null;
    }
  ...
  async removeItem(key: string): Promise<void> {
    try {
      ...
      if (__DEV__) {
        console.error(
          `❌ Erreur lors de la suppression sécurisée de ${key}:`,
          error,
        );
      }
      throw error;
  ...
  async setIdToken(id_token: string): Promise<void> {
    await this.setItem("id_token", id_token);
  }

  async getIdToken(): Promise<string | null> {
    return await this.getItem("id_token");
  }
